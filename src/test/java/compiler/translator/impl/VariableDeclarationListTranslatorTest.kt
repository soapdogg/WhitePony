...
class VariableDeclarationListTranslatorTest {
    ...
    fun translateTest() {
        ...
        Mockito.`when`(variableDeclarationTranslator.translate(parsedVariableDeclarationNode)).thenReturn(translatedVariableDeclarationNode)

        val type = "type"
        Mockito.`when`(variableDeclarationListNode.type).thenReturn(type)

        val actual = variableDeclarationListTranslator.translate(variableDeclarationListNode)
